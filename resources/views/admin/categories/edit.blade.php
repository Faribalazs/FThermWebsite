...
<div class="animate-fade-in-up">
    ...
    <div class="mb-4 sm:mb-6">
        <a href="{{ route('admin.product-categories.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-gray-500 hover:text-primary-600 transition-colors duration-200 group">
    ...
    <div class="mb-6 sm:mb-8">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 bg-clip-text text-transparent">Izmeni kategoriju</h1>
                <p class="text-xs sm:text-sm text-gray-500 mt-0.5">{{ $category->name['sr'] ?? $category->name['en'] ?? '' }}</p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ route('admin.product-categories.update', $category) }}" method="POST">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-4 sm:mb-6" x-data="{ langTab: 'sr' }">
            ...
            <div x-show="langTab === 'sr'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="p-4 sm:p-6">
                ...
                <input type="text" id="name_sr" name="name_sr" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white" value="{{ old('name_sr', $category->name['sr'] ?? '') }}">
            ...
            <div x-show="langTab === 'en'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="p-4 sm:p-6">
                ...
                <input type="text" id="name_en" name="name_en" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white" value="{{ old('name_en', $category->name['en'] ?? '') }}">
            ...
            <div x-show="langTab === 'hu'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="p-4 sm:p-6">
                ...
                <input type="text" id="name_hu" name="name_hu" required class="w-full px-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white" value="{{ old('name_hu', $category->name['hu'] ?? '') }}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="slug" name="slug" required class="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 bg-gray-50 focus:bg-white" value="{{ old('slug', $category->slug) }}">
                        </div>
                        <p class="mt-1 text-[10px] sm:text-xs text-gray-400">Samo mala slova, brojevi i crtice</p>
                    ...
                    <div class="flex items-end">
                        <label class="relative inline-flex items-center gap-3 cursor-pointer group p-3 rounded-xl border border-gray-200 bg-gray-50 hover:bg-white transition-all duration-200 w-full has-[:checked]:border-green-200 has-[:checked]:bg-green-50">
                            <input type="checkbox" id="active" name="active" class="sr-only peer" {{ old('active', $category->active) ? 'checked' : '' }}>
                            ...
                            <div>
                                <span class="text-sm font-semibold text-gray-700">Aktivna</span>
                                <p class="text-[10px] text-gray-400">Kategorija je vidljiva na sajtu</p>
                            </div>
                        </label>
                    </div>
                </div>
        ...
        <div class="flex flex-col-reverse sm:flex-row justify-between items-stretch sm:items-center gap-3">
            <a href="{{ route('admin.product-categories.index') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 border-2 border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 transition-all duration-200 font-bold text-sm">
                Otkaži
            </a>
