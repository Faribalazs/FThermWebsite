    ...
    <div class="bg-white rounded-xl shadow-enhanced overflow-hidden border border-gray-100 animate-scale-in">
        ...
        <form action="{{ route('worker.settings.update') }}" method="POST" class="p-4 sm:p-6 lg:p-8">
            ...
            <div class="mb-6">
                ...
                <div class="bg-gradient-to-br from-orange-50 to-amber-50 border border-orange-200 rounded-xl p-4 sm:p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <!-- Invoice Counter Start -->
                        <div>
                            <label class="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-2" for="invoice_counter_start">
                                Početni Broj Fakture
                                <span class="text-red-500">*</span>
                            </label>
                            <div class="relative">
                                <input 
                                    type="number" 
                                    name="invoice_counter_start" 
                                    id="invoice_counter_start" 
                                    value="{{ old('invoice_counter_start', $invoiceCounterStart) }}"
                                    class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all @error('invoice_counter_start') border-red-500 ring-2 ring-red-200 @enderror"
                                    placeholder="1"
                                    min="1"
                                    required
                                >
                            </div>
                            @error('invoice_counter_start')
                                <div class="mt-2 flex items-center gap-2 text-red-600 text-sm">
                                    <svg class="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
                                    </svg>
                            ...
                            @enderror
                            <p class="mt-2 text-xs text-gray-600">
                                Sledeća faktura: <strong class="text-gray-900">{{ substr(date('Y'), -2) }}-{{ old('invoice_counter_start', $invoiceCounterStart) }}</strong>
                            </p>
                        </div>

                        <!-- Preview -->
                        <div class="flex items-center">
                            <div class="bg-white border border-orange-200 rounded-lg p-4 w-full">
                                <p class="text-xs text-gray-500 mb-1">Format broja fakture</p>
                                <p class="text-lg font-bold text-gray-900">{{ substr(date('Y'), -2) }}-<span class="text-orange-600">[broj]</span></p>
                                <p class="text-xs text-gray-500 mt-2">Prve dve cifre su uvek poslednje dve cifre tekuće godine. Broj se automatski uvećava za svaku novu fakturu.</p>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="p-4 sm:p-6 lg:p-8 border-t border-gray-200">
            <div class="mb-4">
                ...
                <p class="text-sm text-gray-600 mb-4">Upravljajte sačuvanim kontaktima za brže popunjavanje radnih naloga i ponuda</p>
            </div>
            <a href="{{ route('worker.contacts.index') }}"
