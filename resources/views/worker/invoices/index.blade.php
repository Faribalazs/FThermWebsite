    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex justify-between items-center mb-6 animate-fade-in">
            <div>
                ...
                <p class="text-gray-600 mt-1">Pregled kreiranih faktura</p>
            </div>
            <div class="flex items-center gap-3 bg-white px-4 py-2 rounded-lg shadow-md border border-gray-200">
                ...
                <span class="text-sm font-semibold text-gray-700">{{ $invoices->total() }} Faktura</span>
            </div>
        </div>

        <!-- Search and Filter Bar -->
        <div class="bg-white rounded-xl shadow-md border border-gray-200 p-6 mb-6 animate-slide-in">
            <form method="GET" action="{{ route('worker.invoices.index') }}" class="space-y-6">
                <!-- Search -->
                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <select name="location" class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                    ...
                    <!-- Date From -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Datum od</label>
                        <input type="date" 
                               name="date_from" 
                    ...
                    <!-- Date To -->
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Datum do</label>
                        <input type="date" 
                               name="date_to" 
                    ...
                    <div>
                        ...
                        <select name="sort_by" class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                            <option value="created_at" {{ request('sort_by') == 'created_at' ? 'selected' : '' }}>Datumu</option>
                            <option value="invoice_company_name" {{ request('sort_by') == 'invoice_company_name' ? 'selected' : '' }}>Imenu</option>
                            <option value="total_amount" {{ request('sort_by') == 'total_amount' ? 'selected' : '' }}>Iznosu</option>
                            <option value="invoice_number" {{ request('sort_by') == 'invoice_number' ? 'selected' : '' }}>Broju fakture</option>
                        </select>
                    </div>
                </div>
                ...
                <!-- Price Range -->
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Cenovni opseg (RSD)</label>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('worker.invoices.index') }}" class="inline-flex items-center gap-2 px-6 py-3 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50 transition-colors">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button type="button" 
                        ...
                        <svg class="w-5 h-5 {{ request('sort_order') == 'asc' ? '' : 'rotate-180' }} transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('worker.work-orders.show', $invoice) }}" 
                                       class="inline-flex items-center gap-1 px-3 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white text-sm font-semibold rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200"
                                       title="Pregled">
                                    ...
                                    <a href="{{ route('worker.work-orders.invoice', $invoice) }}" 
                                       class="inline-flex items-center gap-1 px-3 py-2 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white text-sm font-semibold rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200"
                                       title="Faktura">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
                                        </svg>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-200 p-12 text-center animate-fade-in">
            ...
            <h3 class="text-xl font-semibold text-gray-700 mb-2">Nema Faktura</h3>
            <p class="text-gray-500 mb-4">{{ request()->hasAny(['search', 'location', 'date_from', 'date_to', 'price_from', 'price_to']) ? 'Nema faktura koje zadovoljavaju kriterijume pretrage.' : 'Još uvek nema kreiranih faktura.' }}</p>
            @if(!request()->hasAny(['search', 'location', 'date_from', 'date_to', 'price_from', 'price_to']))
            <a href="{{ route('worker.work-orders.index') }}" class="inline-flex items-center gap-2 mt-4 px-6 py-3 bg-gradient-to-r from-primary-600 to-primary-700 hover:from-primary-700 hover:to-primary-800 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
