    ...
    <div class="p-3 sm:p-6 max-w-6xl mx-auto">
        ...
        <nav class="mb-4 sm:mb-6">
            <ol class="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-gray-600">
                <li><a href="{{ route('worker.work-orders.index') }}" class="hover:text-primary-600 transition">Radni
                        Nalozi</a></li>
        ...
        <div class="bg-white rounded-xl shadow-enhanced overflow-hidden border border-gray-100 animate-scale-in">
            ...
            <div class="bg-gradient-to-r from-primary-600 to-primary-700 px-4 sm:px-8 py-4 sm:py-6">
                <div class="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-primary-100 text-xs sm:text-sm mt-0.5 sm:mt-1">Popunite podatke o radnom nalogu i
                            dodajte usluge</p>
                    </div>
                </div>
            </div>
            ...
            @if (session('error'))
                <div class="m-3 sm:m-6 bg-red-50 border-l-4 border-red-500 p-3 sm:p-4 rounded-lg">
                    <div class="flex items-center">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-red-500 mr-2 sm:mr-3 flex-shrink-0" fill="currentColor"
                            viewBox="0 0 20 20">
            ...
            <form action="{{ route('worker.work-orders.store') }}" method="POST" class="p-3 sm:p-8" id="workOrderForm">
                ...
                @include('worker.partials.contact-selector')
                                ...
                                <!-- Warehouse Selection -->
                <div
                    class="mb-6 sm:mb-8 bg-gradient-to-r from-gray-50 to-gray-100 border-2 border-gray-200 rounded-xl p-4 sm:p-6">
                    ...
                    <p class="text-xs text-gray-600 mb-3">Izaberite skladište iz kojeg će biti umanjene zalihe materijala
                    </p>
                    <div class="custom-select-wrapper">
                        <input type="hidden" name="warehouse_id" id="warehouse_value"
                            value="{{ old('warehouse_id', $primaryWarehouseId ?? $warehouses->first()?->id) }}" required>
                        <div class="custom-select-trigger" onclick="toggleWarehouseDropdown()">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="custom-select-value text-sm sm:text-base" id="warehouse_selected_text">
                                    @php
                                        $selectedWarehouseId = old('warehouse_id', $primaryWarehouseId ?? $warehouses->first()?->id);
                ...
                <div
                    class="mb-6 sm:mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl p-4 sm:p-6">
                    <label class="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-3">
                        <svg class="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label
                            class="relative flex items-center justify-center p-4 border-2 rounded-lg cursor-pointer transition-all hover:border-primary-400 has-[:checked]:border-primary-600 has-[:checked]:bg-primary-50 has-[:checked]:shadow-lg">
                ...
                <div class="grid grid-cols-1 gap-4 sm:gap-6 mb-6 sm:mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                @error('client_name')
                                    <div class="mt-1 sm:mt-2 flex items-center gap-1 sm:gap-2 text-red-600 text-xs sm:text-sm">
                                        <svg class="w-3 h-3 sm:w-4 sm:h-4 flex-shrink-0" fill="currentColor"
                                            viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                                d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z"
                                                clip-rule="evenodd"></path>
                            ...
                            <div>
                                ...
                                <input type="text" name="client_address" id="client_address"
                                    ...
                                    class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all @error('client_address') border-red-500 ring-2 ring-red-200 error @enderror"
                    ...
                    <div id="pravno_lice_fields" style="display: none;">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" name="pib" id="pib" value="{{ old('pib') }}"
                                    class="form-input w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all"
                            ...
                            <div>
                                ...
                                <input type="text" name="maticni_broj" id="maticni_broj"
                    ...
                    <div>
                        ...
                        <input type="text" name="location" id="location" value="{{ old('location') }}"
                            ...
                            placeholder="Unesite lokaciju" required>
                        @error('location')
                            <div class="mt-1 sm:mt-2 flex items-center gap-1 sm:gap-2 text-red-600 text-xs sm:text-sm">
                ...
                if (parseInt(input.value) === productId) {
                    ...
                    if (match) {
                        ...
                        const quantityInput = document.querySelector(
                            `input[name="sections[${sectionId}][items][${itemId}][quantity]"]`);
                        if (quantityInput) {
        ...
        <div class="item-row bg-white border border-gray-300 rounded-lg p-3" data-item="${itemId}">
            <div class="mb-2">
                <div class="flex items-center justify-between mb-1">
                    <label class="text-sm font-semibold text-gray-700">Materijal *</label>
            ...
            <div class="flex gap-2">
                <div class="w-28 shrink-0">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Količina</label>
                    <input 
                        type="number" 
        ...
        document.getElementById('workOrderForm').addEventListener('submit', function(event) {
            ...
            quantityInputs.forEach(input => {
                ...
                if (!isNaN(stock) && quantity > stock) {
                    ...
                    const productName = itemRow ? itemRow.querySelector('.custom-select-value')
                        .textContent : 'Materijal';
