    ...
    <div class="bg-white rounded-xl shadow-enhanced overflow-hidden border border-gray-100 animate-scale-in">
        ...
        <div class="p-4 sm:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-6 sm:mb-8 pb-6 sm:pb-8 border-b border-gray-200">
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div class="text-gray-900">
                        <p class="font-bold text-base sm:text-lg">F-Therm d.o.o.</p>
                        <p class="text-xs sm:text-sm mt-2">Industrijska 15</p>
                        <p class="text-xs sm:text-sm">Beograd, Srbija</p>
                        <p class="text-xs sm:text-sm mt-2">PIB: 123456789</p>
                        <p class="text-xs sm:text-sm">Matični broj: 987654321</p>
                    </div>
                </div>
                ...
                <div class="bg-light-50 p-4 rounded-lg">
                    ...
                    <div class="text-gray-900">
                        <p class="font-bold text-base sm:text-lg">{{ $workOrder->invoice_company_name }}</p>
                        @if ($workOrder->invoice_pib)
                        <p class="text-xs sm:text-sm mt-2">PIB: {{ $workOrder->invoice_pib }}</p>
                        @endif
                        <p class="text-xs sm:text-sm {{ $workOrder->invoice_pib ? '' : 'mt-2' }}">{{ $workOrder->invoice_address }}</p>
                        @if ($workOrder->invoice_email)
                        <p class="text-xs sm:text-sm mt-2">Email: {{ $workOrder->invoice_email }}</p>
                        @endif
                        @if ($workOrder->invoice_phone)
                        <p class="text-xs sm:text-sm">Tel: {{ $workOrder->invoice_phone }}</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="mb-6 bg-gradient-to-r from-indigo-50 to-blue-50 p-4 rounded-lg border border-indigo-100">
                ...
                <p class="text-gray-900 text-sm sm:text-base"><span class="font-semibold">Klijent:</span> {{ $workOrder->client_name }}</p>
                <p class="text-gray-900 text-sm sm:text-base"><span class="font-semibold">Lokacija:</span> {{ $workOrder->location }}</p>
            </div>
            ...
            <div class="border-t-2 border-gray-300 pt-6 pb-6">
                <div class="w-full sm:w-80 ml-auto">
                    ...
                    <div class="flex justify-between items-center py-3 sm:py-4 bg-gradient-to-r from-indigo-50 to-blue-50 px-3 sm:px-4 rounded-lg mt-3 border-2 border-indigo-200">
                        ...
                        <span class="text-xl sm:text-2xl font-bold text-indigo-600">{{ number_format($workOrder->calculateGrandTotal(), 2) }} RSD</span>
                    </div>
                    @else
                    <div class="flex justify-between items-center py-2 sm:py-3 border-b border-gray-200 text-sm sm:text-base">
                        <span class="text-gray-600">Osnova:</span>
                    ...
                    <div class="flex justify-between items-center py-3 sm:py-4 bg-gradient-to-r from-indigo-50 to-blue-50 px-3 sm:px-4 rounded-lg mt-3 border-2 border-indigo-200">
                        <span class="text-lg sm:text-xl font-bold text-gray-900">UKUPNO:</span>
                        <span class="text-xl sm:text-2xl font-bold text-indigo-600">{{ number_format($workOrder->total_amount, 2) }} RSD</span>
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="text-center text-xs sm:text-sm text-gray-500 mt-6 sm:mt-8 pt-6 border-t border-gray-200">
                <p class="font-medium">Hvala Vam na poverenju!</p>
                <p class="mt-1">Za sva pitanja kontaktirajte nas na:</p>
                <p class="mt-1">info@ftherm.rs | +381 11 123 4567</p>
            </div>
