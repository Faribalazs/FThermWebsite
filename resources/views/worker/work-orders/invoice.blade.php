...
<div class="p-3 sm:p-6 max-w-5xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-enhanced overflow-hidden border border-gray-100 animate-scale-in">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                ...
                <div class="flex flex-wrap gap-2 w-full sm:w-auto">
                    <a href="{{ route('worker.work-orders.invoice.download', $workOrder) }}" target="_blank"
                       class="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 bg-white/15 backdrop-blur-sm text-white ring-1 ring-white/20 px-4 py-2.5 rounded-xl font-semibold hover:bg-white/25 transition-all text-sm active:scale-[0.98]">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        PDF
                    </a>
                    @if($workOrder->client_type === 'pravno_lice' && (!$workOrder->efaktura_status || $workOrder->efaktura_status === 'error'))
                    <form id="efaktura-form" method="POST" action="{{ route('worker.work-orders.invoice.send-efaktura', $workOrder) }}" class="flex-1 sm:flex-initial">
                        @csrf
                        <button type="button" onclick="document.getElementById('efaktura-confirm-modal').classList.remove('hidden')" class="w-full inline-flex items-center justify-center gap-2 bg-white text-primary-700 px-4 py-2.5 rounded-xl font-semibold hover:bg-gray-100 transition-all shadow-lg text-sm active:scale-[0.98]">
        ...
        <div class="p-4 sm:p-8">
                ...
                <div class="mb-5 last:mb-0">
                    ...
                    <div class="hidden sm:block rounded-xl border border-gray-200 overflow-hidden">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead>
                                <tr class="bg-gray-50">
                                    <th class="px-5 py-2.5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider w-8">#</th>
                                    <th class="px-5 py-2.5 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Naziv</th>
                                    <th class="px-5 py-2.5 text-center text-xs font-bold text-gray-500 uppercase tracking-wider">Kol.</th>
                                    <th class="px-5 py-2.5 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Jed. cena</th>
                                    <th class="px-5 py-2.5 text-right text-xs font-bold text-gray-500 uppercase tracking-wider">Iznos</th>
                                </tr>
                            </thead>
            ...
            {{-- ===== Totals ===== --}}
            <div class="bg-gradient-to-br from-gray-50 to-gray-100/80 rounded-2xl p-4 sm:p-6 border border-gray-200/60">
                ...
                <div class="space-y-2">
                    ...
                    {{-- Amount in words --}}
                    <div class="rounded-lg bg-white/80 border border-gray-200/60 px-4 py-2.5 mt-2">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mt-6 pt-6 border-t border-gray-100">
                <a href="{{ route('worker.work-orders.show', $workOrder) }}" class="inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition-all active:scale-[0.98]">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                    Radni Nalog
                </a>
                <a href="{{ route('worker.invoices.index') }}" class="inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-xl transition-all active:scale-[0.98]">
