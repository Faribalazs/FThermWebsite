        ...
        <div class="bg-white rounded-xl shadow-enhanced overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300 card-hover animate-scale-in work-order-card" data-index="{{ $i }}">
            <div class="bg-gradient-to-r from-primary-600 to-primary-700 p-4">
                <div class="flex justify-between items-start">
                    ...
                    <span class="bg-gray-400 text-white text-xs font-semibold px-3 py-1 rounded-full">
                        Nacrt
                    </span>
                    @elseif ($workOrder->has_invoice)
            ...
            <div class="p-4">
                <div class="space-y-3">
                    ...
                    <div class="flex items-center text-gray-600">
                        ...
                        <span class="text-sm">{{ $workOrder->sections_count }} {{ $workOrder->sections_count == 1 ? 'usluga' : 'usluga' }}</span>
                    </div>
                ...
                <div class="mt-4 flex gap-2">
                    @if ($workOrder->status === 'draft')
                    <a href="{{ route('worker.work-orders.edit', $workOrder) }}" class="flex-1 inline-flex items-center justify-center gap-1 px-3 py-2 bg-amber-50 text-amber-700 rounded-lg hover:bg-amber-100 transition-all action-btn text-sm font-medium">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                        </svg>
                        Nastavi
                    </a>
                    <form method="POST" action="{{ route('worker.work-orders.destroy', $workOrder) }}" class="delete-form" data-title="Obriši nacrt" data-text="Radni nalog br. {{ $workOrder->id }} će biti trajno obrisan.">
                        @csrf
                        @method('DELETE')
